...
## 1. Введение в проектирование интерфейсов
...
- Изучить типы и виды интерфейсов.

- Объяснить значение и UI и UX в современном цифровом мире и процессе разработки продуктов.
...
<dfn title="интерфейс">Интерфейс</dfn> — это общий термин, обозначающий средство или набор правил, с помощью которых происходит взаимодействие между двумя или более системами, компонентами или субъектами (агентами). Другими словами, <dfn title="интерфейс">интерфейс</dfn> — механизм, обеспечивающий взаимодействие пользователя с приложением.
...
В зависимости от того, как представлен интерфейс и как пользователь с ним взаимодействует, различают несколько видов интерфейсов. Каждый вид обладает своими особенностями и используется в зависимости от контекста, задачи и пользователей. Выбор вида интерфейса важен для создания удобного и эффективного взаимодействия с системой.

1. <dfn title="текстовый пользовательский интерфейс">Текстовый пользовательский интерфейс</dfn> (TUI или CUI) — разновидность интерфейса пользователя, использующая при вводе-выводе и представлении информации исключительно набор буквенно-цифровых символов и символов псевдографики. Для управления системы и взаимодействия с ней используется текст или программный код, при этом система также отвечает текстом или кодом. Может представлять собой промежуточный тип между графическим и командным, использует текстовые элементы и символы для взаимодействия, часто применяем в серверных системах и специализированных приложениях. Программы с текстовым интерфейсом могут реализовывать оконный интерфейс (Norton Commander), чему особенно способствует применение псевдографических символов.
...
2. <dfn title="графический пользовательский интерфейс">Графический пользовательский интерфейс</dfn> (GUI) — разновидность интерфейса пользователя, способ взаимодействия пользователя с компьютером, когда диалог человека и компьютера основан на манипуляции динамически выводимыми на экран объектами. Это наиболее распространенный тип интерфейса, основанный на визуальных элементах — окнах, кнопках, иконках, меню, которыми можно управлять с помощью мыши, стилуса или пальцев рук. Взаимодействие происходит с помощью мыши, клавиатуры или сенсорного экрана. Примеры — операционные системы (Windows, macOS), веб-браузеры, офисные программы.

    Для выбора объекта и взаимодействия с ним может использоваться как прямое указание на объект с помощью тачскрина или светового пера, так и косвенное посредством курсора, управляемого с помощь мыши, клавиатуры или другого устройства. Элементы, с которыми пользователь может взаимодействовать, называются <dfn title="виджет">виджетами</dfn>. Таким образом, графический интерфейс — это тот, в оформлении которого задействованы все графические средства — цвета, стили, формы. Им обладает большинство приложений. В каждом случае настройки интерфейса пользователя могут быть индивидуальными (темная или светлая тема, определенная цветовая гамма, цвет шрифта, стили кнопок), но при этом базовые компоненты остаются неизменными.
   ...
   - **Зависимость от аппаратного обеспечения**: GUI более требователен к мощности компьютера, что может быть проблематично для слабых или старых устройств, где производительность будет снижена.
    ...
    Графические интерфейсы можно выделить на три основных типа в зависимости от платформы (где и для чего используются):
   ...
   - **Мобильные интерфейсы** — интерфейсы мобильных приложений, с которыми пользователи взаимодействуют с экрана смартфона или планшета, при этом для удобного взаимодействия с системой используются жесты (свайпы, зум). Такие интерфейсы адаптированы для смартфонов и планшетов с учётом небольших экранов и сенсорного управления. Отсюда их особенности: крупные элементы интерфейса, блоки меню, кнопки, иконки, заполняющие экран полностью так, чтобы их было хорошо видно даже при маленьком разрешении. Расположение элементов обосновано анатомически так, чтобы ими было удобно управлять с помощью одной руки, а пальцы не промахивались мимо кнопок.  Типичные примеры — интерфейсы iOS и Android приложений.

   - **Веб-интерфейсы** — интерфейсы сайтов, веб-приложений и онлайн-сервисов, которые работают в классических браузерах. Состоят из навигационных элементов и интерактивных компонентов. От мобильных их отличает то, что композиции их элементов может быть любой, а управляют ими с помощью мыши или сенсорной панели. В таких графических интерфейсах чаще встречаются большие текстовые блоки, видео и анимации. Часто реализуют адаптивный дизайн, чтобы интерфейс корректно отображался на различных устройствах.

   - **Игровые интерфейсы** — графические интерфейсы игр, созданные так, чтобы игрок сразу понимал, куда нужно двигаться и какие возможности у него есть. Главные меню здесь отводится персонажам игры и вспомогательным средствам (машины, роботы, карты и т.д.). Текстовые блоки минимальны и обычно используются только в качестве подсказок. Элементы меню часто стилизованы под сюжет игры.

3. <dfn title="голосовой интерфейс">Голосовой интерфейс</dfn> (VUI) позволяет людям взаимодействовать с устройствами и программами с помощью голосовых команд. Он распознает речь, чтобы понять запросы и команды пользователя, а затем использует синтез речи, чтобы ответить. Таким образом, пользователь взаимодействует с устройством или системой с помощью голоса, а все управление происходит голосовыми командами. Данный вид интерфейса используется в голосовых помощниках (Siri, Alexa, «Алиса»), умных колонках, элементах умного дома, голосых системах управления автомобилем и предоставляет достаточно естественное взаимодействие с системой, что особенно удобно, когда руки заняты или традиционные устройства ввода неудобны.
...
5. <dfn title="жестовый интерфейс">Жестовый интерфейс</dfn> (<dfn title="сенсорный интерфейс">сенсорный интерфейс</dfn>) — подмножество системы ввода для графического пользовательского интерфейса для устройств, оснащённых специальными либо устройствами ввода (отличными от клавиатуры), либо сенсорными экранами, и позволяющая эмулировать клавиатурные команды (либо сочетания клавиш) при помощи жестов (росчерков, англ. gesture). Другими словами, данные интерфейсы используют сенсорные жесты и движения для управления системой, при этом управление системой производится с помощью жестов рук, а иногда еще и движениями тела и головы, ввод обычно осуществляется с помощью специальных контроллеров (джойстиков или перчаток). Применяются в устройствах с сенсорными экранами и технологиями распознавания движения (например, Microsoft Kinect), онлайн-играх, VR-системах. Основной мотивацией разработки таких интерфейсов является улучшение эргономичности управления, с отказом от привычного для компьютерных программ меню приложения.

    - <dfn title="тактильный интерфейс">Тактильный интерфейс</dfn> — компьютерная технология, основанная на осязании. Для управления используется движение пальцев рук по экрану, ввод при этом осуществляется с помощью сенсорной панели. Эти технологии могут использоваться для создания виртуальных объектов в компьютерном моделировании, для управления виртуальными объектами и для улучшения удаленного управления машинами и устройствами (телероботика). Слово haptic, от греческого :ἁπτικός (haptikos) означает «тактильное, относящееся к осязанию». Данный тип интерфейса широко распространен в системах навигации в больших торговых центрах, аэропортах, музеях и т.п.

        Тактильная технология облегчает исследование того, как работает осязание человека, позволяя создавать управляемые тактильные виртуальные объекты. Большинство исследователей выделяют три сенсорные системы, связанные с осязанием человека: *кожную*, *кинестетическую* и *тактильную*. Все восприятия, опосредованные кожной и кинестетической чувствительностью, называются <dfn title="тактическое восприятие">тактическим восприятием</dfn>. Ощущение прикосновения можно разделить на пассивное и активное а термин «тактильный» часто ассоциируется с активным прикосновением для общения или распознавания объектов. Обычны простые тактильные устройства в виде игровых контроллеров (джойстиков, рулей). Тактильные устройства могут включать в себя тактильные датчики, которые измеряют силы, прикладываемые пользователем к интерфейсу.
...
6. <dfn title="нейрокомпьютерный интерфейс">Нейрокомпьютерный интерфейс</dfn> (НКИ) (BCI) (называемый также прямой нейронный интерфейс, мозговой интерфейс, интерфейс «мозг — компьютер») — система, созданная для обмена информацией между мозгом и электронным устройством (например, компьютером). Нейроинтерфейс, или интерфейс «мозг-компьютер», — это технология, позволяющая напрямую обмениваться информацией между мозгом и внешними устройствами, такими как компьютеры и протезы, обходя органы чувств. Эта система регистрирует активность мозга (например, с помощью ЭЭГ) и переводит ее в команды для управления устройствами, а также может передавать сигналы в мозг, например, имитируя зрение. Таким образом, системой можно управлять с помощью нейронов мозга, т.е. буквально силой мысли. Такие интерфейсы, например, используются в управлении протезами для пациентов с травмами позвоночника, ДЦП и т.п.

    В однонаправленных интерфейсах внешние устройства могут либо принимать сигналы от мозга, либо посылать ему сигналы (например, имитируя сетчатку глаза при восстановлении зрения электронным имплантатом). Двунаправленные интерфейсы позволяют мозгу и внешним устройствам обмениваться информацией в обоих направлениях. В основе нейрокомпьютерного интерфейса часто используется метод биологической обратной связи.[^Нейрокомпьютерный_интерфейс]

    **Как это работает**
    - **Регистрация сигналов**: система записывает электрическую активность мозга пользователя.
    - **Расшифровка**: специальные алгоритмы анализируют и интерпретируют эти сигналы, определяя намерения человека.
    - **Передача команд**: расшифрованные команды передаются на электронное устройство, позволяя управлять им силой мысли.

    Нейроинтерфейсы делятся на два основных типа по степени инвазивности:
    - **Неинвазивные**: cчитывают сигналы мозга снаружи, без хирургического вмешательства. К ним относятся, например, электроэнцефалографы (ЭЭГ).
    - **Инвазивные**: требуют имплантации электродов или чипов непосредственно в мозг для более точной регистрации сигналов.

    **Области применения**
    - **Медицина и реабилитация**: наиболее распространенное применение — помощь парализованным людям управлять компьютерами и протезами.
    - **Восстановление функций**: например, глазные протезы, которые подключаются к зрительному нерву, чтобы вернуть зрение.
    - **Расширение возможностей**: в перспективе — управление «умным» домом, виртуальной реальностью, а также новые способы взаимодействия с информацией.

    **Ключевые разработки**
    - **Neuralink**: Компания Илона Маска разрабатывает инвазивные чипы с «нейронитями», которые вживляются в мозг для управления устройствами.
    - **Synchron**: Австралийская компания предлагает «стентроды» (stentrode), которые устанавливаются в мозг через кровеносные сосуды.
    - **Blackrock Neurotech**: Один из крупнейших производителей инвазивных нейроинтерфейсов, чьи технологии используются в экспериментах с парализованными людьми.

*[ПО]: программное обеспечение

Таким образом, графический интерфейс может быть одновременно и тактильным (планшеты), и жестовым и нейронным. На сегодняшний день главным инструментом общения человека с информационной средой является удобный и понятный графический интерфейс. Именно он помогает пользователю осуществлять взаимодействие с машинным кодом. Без удобной организации взаимодействия даже самая искусное ПО может остаться невостребованным.

### Требования к качественному интерфейсу
<dfn title="Графический интерфейс пользователя">Графический интерфейс пользователя</dfn> определяется как разновидность пользовательского интерфейса, в котором элементы интерфейса (меню, кнопки, значки, списки и т.д.), представленные пользователю на дисплее, исполнены в виде графических изображений. А следовательно, графический интерфейс определяет взаимодействие с пользователем на уровне визуальной информации. Дизайн такой информации должен способствовать максимально простому и продуктивному взаимодействию с пользователем, быть ориентированным на пользователя. Рациональная конструкция интерфейса приложения обеспечивает простоту работы и играет важную роль в создании приложения. Интерфейс должен, прежде всего, учитывать потребности конечного пользователя. **Неудачный графический интерфейс** снижает эффективность работы с приложением и вызывает у пользователей разочарование.
...
1. **Понятность и однозначность**: интерфейс должен быть максимально ясным и однозначным для пользователя. Все элементы, надписи и действия должны быть легко воспринимаемы и интерпретируемы, чтобы не вызывать вопросов и сомнений при использовании. Важнейшим качеством является быстрая узнаваемость — интерфейс должен стать понятен для пользователя за 3 секунды.
   ...
   - все элементы, надписи и действия легко воспринимаемы и интерпретируемы;
   - оформление отвечает заявленной тематике;
   - внешний вид соответствует целевой аудитории;
   - интерфейс становится понятным за время, не превышающее 3 секунд.
...
2. **Последовательность и единообразие**: элементы интерфейса должны быть консистентными — одинаковые действия должны выполняться одинаково во всех частях системы. Использование общепринятых стандартов и паттернов улучшает понимание и повышает скорость обучения. Ход действий в интерфейсе должен быть последовательным и понятным, соответствовать ожиданиям пользователя. Навигация и структура интерфейса должны отражать логику выполнения задач, чтобы не вызывать путаницу. Работа с интерфейсом должна быть последовательной и понятной, отражать логику выполнения задач, чтобы пользователь не испытывал затруднений при навигации и выполнении действий.
   ...
   - ход действий в интерфейсе последователен и логичен;
   - навигация и структура интерфейса отражают логику выполнения задач;
   - поиск важной информации не превышает 3 клика.
...
3. **Предсказуемость и интуитивность**: интерфейс должен быть интуитивно понятным, чтобы даже новые пользователи могли легко разобраться с его функционалом. Все элементы управления должны быть логично расположены и легко доступны. Согласно концепции DWIM (Do What I Mean, "делай то, что я имею в виду"), интерфейс должен работать предсказуемо. Пользователь должен интуитивно понимать, какое действие выполнит система в ответ на его команду без необходимости дополнительного изучения или уточнения. Это позволяет минимизировать ошибки и ускоряет взаимодействие.
   ...
   - элементы управления логично расположены и легко доступны;
   - элементы управления выполняют предписанные функции.
...
4. **Наглядность и экспрессивность**: Интерфейс должен использовать выразительные средства (цвет, шрифты, иконки, анимации) для акцентирования внимания на важных элементах и поддержания эмоционального контакта с пользователем. Это помогает создавать подробнее значимый опыт и усиливает восприятие. Интерфейс должен использовать цвет, шрифты, иконки и другие визуальные средства для акцентирования важной информации и создания эмоционального отклика, что способствует лучшему восприятию и запоминанию.
   ...
   - интерфейс использует выразительные средства (цвет, шрифты, иконки, анимации) для акцентирования важной информации;
   - деление интерфейса на функциональные зоны для облегчения восприятия и повышения удобочитаемости;
   - применение анимаций и визуальных переходов для плавного сопровождения изменений и облегчения понимания происходящих процессов.
...
5. **Лаконичность и эстетика**: интерфейс должен быть простым, без излишних элементов, которые отвлекают или усложняют работу. Важно избегать перегруженности визуальными элементами и информацией, минимизируя когнитивную нагрузку на пользователя. Минимализм помогает сосредоточить внимание на главном, облегчает обучение и предотвращает перегрузку внимания пользователя. Привлекательный дизайн повышает удовольствие от использования. При этом визуальные элементы должны создавать четкую иерархию, помогая быстро находить и воспринимать нужную информацию.
   ...
   - визуальные элементы создают четкую иерархию, помогая быстро находить и воспринимать нужную информацию;
   - интерфейс не содержит излишних или нефункциональных элементов;
   - внешний вид не перегружен избыточными деталями.
...
6. **Отзывчивость и контролируемость**: система должна своевременно информировать пользователя о результатах его действий, об ошибках, необходимости ожидания или успешном завершении операций. Это снижает уровень стресса и увеличивает доверие к интерфейсу. Интерфейс должен быстро реагировать на действия пользователя, обеспечивать своевременную обратную связь, информируя о выполнении или ошибках, что повышает доверие и комфорт использования. Пользователю необходимо предоставить возможность контролировать свои действия в системе: отменять ошибки, возвращаться назад, получать помощь и подсказки. Интерфейс не должен навязывать пользователю жесткие сценарии, а должен давать свободу выбора.
   ...
   - интерфейс быстро реагирует на действия пользователя, обеспечивая своевременную обратную связь;
   - система своевременно информирует об ошибках, необходимости ожидания или успешном завершении операций;
   - имеется возможность на каждом этапе контролировать действия в системе (отменять ошибки, возвращаться назад, получать помощь и подсказки);
   - интерфейс не навязывает пользователю жесткие сценарии, предоставляя свободу выбора в любой момент времени.
...
7. **Гибкость и адаптивность**: интерфейс должен эффективно работать на различных устройствах и платформах. Он должен поддерживать разные способы ввода (клавиатура, мышь, сенсор, голос), а также и подстраиваться под потребности пользователя для удобства и доступности, позволяя пользователем настраивать некоторые параметры под свои нужды (цветовая схема, размер шрифтов и пр.) .
   ...
   - интерфейс эффективно работает на различных устройствах и платформах;
   - интерфейс поддерживает разные способы ввода (клавиатура, мышь, сенсор, голос);
   - предоставляется возможность настраивать параметры интерфейса под свои нужды (цветовая схема, размер шрифтов и пр.).
...
8. **Открытость и доступность**: интерфейс должен быть доступен для максимального числа пользователей, включая людей с ограниченными возможностями, а также тех людей, чьи возможности ограничены временно (управление транспортом и другими процессами). Это включает правильный выбор цветов, поддержку экранных читалок, возможность управления без мыши и другие особенности. Создание открытого и доступного интерфейса повышает комфорт, безопасность и эффективность работы для всех пользователей, расширяя аудиторию и соответствуя современным нормативным требованиям и этическим стандартам
   ...
   - интерфейс дает возможность управления без мыши (клавиатурное, голосовое);
   - поддерживается использование программ экранного доступа (чтения с экрана);
   - предоставляется режим для слабовидящих.
...
9. **Производительность и эффективность**: дизайн должен способствовать выполнению задач пользователя с минимальными затратами времени и усилий, обеспечивая эффективное взаимодействие с системой. Это касается как скорости работы системы, так и оптимальной организации самой работы в интерфейсе. Эффективный интерфейс подразумевает минимизацию количество действий для выполнения задач.
   ...
   - система может быть приведена в состояние готовности за минимальное время;
   - пользовательские действия обрабатываются максимально эффективно;
   - навигационные переходы осуществляются достаточно быстро.
...
10. **Безопасность и надежность**: интерфейс должен защищать пользователя от случайных ошибок и обеспечивать безопасность личных данных. Важна предсказуемость работы и минимизация рисков. Надежность пользовательского интерфейса — это комплексное свойство, направленное на обеспечение устойчивой, безопасной и предсказуемой работы, создающей комфортные условия для пользователя и минимизирующей риски при взаимодействии с системой.
    ...
    - используется безопасное соединение (сертификаты) и защищенный канал связи (шифрование);
    - обеспечивается безопасный ввод чувствительной информации (скрытный ввод и маскирование паролей);
    - осуществляется надежная защита пользовательских данных (шифрование паролей и секретов стойкими криптографическими алгоритмами);
    - организован комплекс мероприятий для повышения надежности хранения данных (архивирование и резервное копирование);
    - гарантируется восстановление рабочего состояния и данных системы после сбоев и отказов;
    - предприняты меры по защите пользовательских и чувствительных данных от взлома и утечек.
...
### UI/UX-дизайн
Таким образом:
- **UI (User Interface)** отвечает за внешний вид и интерактивность продукта — это то, как продукт выглядит. UI — это все, с чем пользователь взаимодействует в процессе использования цифрового продукта, такого как веб-сайт или приложение. UI включает визуальные элементы, такие как кнопки, меню, шрифты, цвета, изображения и иконки, которые позволяют человеку управлять системой, получать информацию и выполнять задачи. Таким образом, **UI-дизайнер** отвечает за визуальное оформление. В зону его ответственности входит:

  - визуальная часть дизайна приложения;
  - определение цветовой гаммы;
  - отрисовка графических элементов;
  - подбор шрифтов;
  - отрисовка макета с размещением всех визуальных элементов на странице.

- **UX (User Experience)** отвечает за то, насколько удобно, логично и приятно пользоваться продуктом — это ощущение от использования. UX — это совокупность ощущений, впечатлений и эмоций, которые человек получает при взаимодействии с продуктом, сервисом или услугой. Понятие включает в себя удобство использования, функциональность, доступность и привлекательность продукта. Цель UX-дизайна — спроектировать такой опыт, чтобы пользователь мог легко и эффективно достигать своих целей, используя позитивные эмоции. СоответственнО, **UX-дизайнер** отвечает за то, как интерфейс работает — выстраивает структуру приложения так, чтобы пользователю было удобнее двигаться к цели. Он ответственнен за следующие аспекты:

  - тестирование удобства пользования;
  - информационная архитектура;
  - определение сценариев поведения пользователей на странице;
  - каркасная структура и прототипирование приложения;
  - проектирование интерфейса на основе исследования, направленных на понимание поведения пользователя.

Таким образом, **UX — это функционал интерфейса, UI — его внешний вид**. UX — это весь процесс взаимодействия пользователя с продуктом, как он его использует, насколько легко ему было достичь цели, какие эмоции он испытывает. UI — это лишь визуальная часть интерфейса, которая делает продукт привлекательным, но не гарантирует хорошего пользовательского опыта. UI является частью общего пользовательского опыта, делая его приятным и понятным.
...
[^Нейрокомпьютерный_интерфейс]: [Нейрокомпьютерный интерфейс](https://ru.wikipedia.org/wiki/%D0%9D%D0%B5%D0%B9%D1%80%D0%BE%D0%BA%D0%BE%D0%BC%D0%BF%D1%8C%D1%8E%D1%82%D0%B5%D1%80%D0%BD%D1%8B%D0%B9_%D0%B8%D0%BD%D1%82%D0%B5%D1%80%D1%84%D0%B5%D0%B9%D1%81)
